...
from .mapcss import MapCSS, webcolors
...
def main(argv=None):
    ...
    if options.renderer == "mapbox-style-language":
        ...
        from . import libkomb
        libkomb.komap_mapbox(options, style)
        return

    if options.renderer == "mvt-sql":
        from . import mvt_sql
        mvt_sql.komap_mvt_sql(options, style)
        return

    if options.renderer == "mapswithme":
        from . import libkomwm
        libkomwm.komap_mapswithme(options, style, options.filename)
        return
    ...
    if options.renderer == "mapnik":
        ...
        globals().update({
            name: getattr(libkomapnik, name)
            for name in dir(libkomapnik)
        ...
        if options.osm2pgsqlstyle != "-":
            ...
            for line in mf:
                ...
                if line and line[0][0] != "#" and "phstore" not in line:
